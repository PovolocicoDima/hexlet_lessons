...
export default async () => {
    ...
    // каждый новый запуск создает свой собственный объект инстанса и не меняет глобальный объект
    ...
    const watchedState = onChange(state, (path, value) => {
        switch (path) {
            case 'lng':
                i18nInstance.changeLanguage(value).then(() => render(container, watchedState, i18nInstance));
                break;
            case 'clicksCount':
